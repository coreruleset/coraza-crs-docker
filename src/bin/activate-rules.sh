...
setup_conf_path="/opt/coraza/config/crs-setup.conf"
...
if [ -n "${CORAZA_DEFAULT_PHASE1_ACTION}" ]; then
  sed -z -E -i "s/SecDefaultAction \"phase:1,log,auditlog,pass\"/SecDefaultAction \"${CORAZA_DEFAULT_PHASE1_ACTION}\"/" "${setup_conf_path}"
fi

if [ -n "${CORAZA_DEFAULT_PHASE2_ACTION}" ]; then
  sed -z -E -i "s/SecDefaultAction \"phase:2,log,auditlog,pass\"/SecDefaultAction \"${CORAZA_DEFAULT_PHASE2_ACTION}\"/" "${setup_conf_path}"
fi
